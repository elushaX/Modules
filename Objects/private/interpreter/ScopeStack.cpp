...
Scope::~Scope() {
	for (auto local : mLocals) {
		context->destroy(local->val);
	}
	for (auto tmp : mTemps) {
		context->destroy(tmp.data());
	}
}
...
void ScopeStack::addTemp(obj::Object* tmp) {
	ObjectsContext::increaseReferenceCount(tmp);
	mBuff[mIdx - 1].mTemps.pushBack(tmp);
}

void ScopeStack::popTemp() {
	context->destroy(mBuff[mIdx - 1].mTemps.last()->data);
	mBuff[mIdx - 1].mTemps.popBack();
}

void ScopeStack::addTempReturn(obj::Object* ret) {
	if (mIdx >= 2) {
		ObjectsContext::increaseReferenceCount(ret);
...
void ScopeStack::addLocal(obj::Object* local, const std::string& id) {
	...
	if (idx) {
		context->destroy(locals.getSlotVal(idx));
	}
	ObjectsContext::increaseReferenceCount(local);
	locals.put(id, local);
}
...
Scope* ScopeStack::getCurrentScope() const { return &mBuff[mIdx - 1]; }
